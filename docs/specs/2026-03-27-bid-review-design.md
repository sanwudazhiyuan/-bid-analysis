...
- **上传后自动执行信息脱敏**（姓名、手机号、身份证号、邮箱、银行账号），审查全程不还原隐私数据
- **自动提取文档中的嵌入图片**（如资格证书扫描件），在预览界面中展示供人工审阅
...
自动检查招标文件解析状态:
  ...
  ↓
信息脱敏:
  ├── 正则匹配手机号、身份证号、邮箱、银行账号
  ├── 上下文匹配姓名（联系人、项目经理等关键词后的中文名）
  └── 替换为编号占位符 [姓名_1] [电话_1] 等，相同值复用同一占位符
  ↓
图片提取:
  ├── docx → 解压 word/media/ 目录提取图片，通过 document.xml.rels 映射到段落位置
  └── PDF → 使用 pymupdf 逐页提取嵌入图片（可选，graceful degradation）
  ↓
投标文件索引:
  ├── 检测到目录页 → 按目录结构分段建索引
...
- **隐私保护优先**：上传后第一步执行 PII 脱敏，后续所有处理（索引、LLM 审查、报告生成）均基于脱敏后的文本。LLM prompt 明确禁止还原或猜测脱敏内容
- **图片感知审查**：自动提取文档中的嵌入图片，在段落文本中注入 `[图片: filename]` 标记使 LLM 知晓图片存在，在前端预览中内联展示图片
...
投标文件提取的图片存放在 `images/` 子目录中，图片元数据（filename, near_para_index, content_type）存储在 `review_summary.extracted_images` 中。
...
在 `parse_document()` 之后、TOC 检测/索引构建之前执行。所有后续处理（索引、LLM 审查、报告生成）均基于脱敏后的文本。
...
| 类别   | 检测方式                                                                             | 占位符格式      | 示例                                               |
| ---- | -------------------------------------------------------------------------------- | ---------- | ------------------------------------------------ |
| 手机号  | 正则 `1[3-9]\d{9}`                                                                 | `[电话_N]`   | 13812345678 → [电话_1]                             |
| 身份证号 | 正则（含日期校验）`[1-9]\d{5}(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])\d{3}[\dXx]` | `[身份证_N]`  | 110101199003071234 → [身份证_1]                     |
| 邮箱   | 正则匹配                                                                             | `[邮箱_N]`   | [zs@example.com](mailto:zs@example.com) → [邮箱_1] |
| 银行账号 | 正则 `\d{16,19}`                                                                   | `[银行账号_N]` | 6222021234567890 → [银行账号_1]                      |
| 姓名   | 上下文关键词 + 2-4 汉字                                                                  | `[姓名_N]`   | 项目经理：张三 → 项目经理：[姓名_1]                            |


**上下文关键词**：联系人、项目经理、项目负责人、法定代表人、授权代表、负责人、经办人、签字人、投标人代表

### 4a.3 脱敏规则

- 相同的原始值复用同一占位符（如同一手机号出现多次，均替换为 `[电话_1]`）
- 身份证优先于银行账号检测（避免 18 位身份证被误识别为银行账号）
- 表格单元格中的数据同样执行脱敏
- 脱敏映射表（placeholder → original）仅在内存中保持，**不持久化到数据库或磁盘**

### 4a.4 LLM Prompt 约束

在 `review_clause.txt` 和 `review_batch.txt` 的 prompt 模板中加入隐私保护规则：

```
## 隐私保护规则（必须遵守）
投标文件中的个人隐私数据已被脱敏处理，以 [姓名_N]、[电话_N] 等占位符表示。
- 禁止填充、还原、猜测任何脱敏占位符的真实内容
- 引用原文时保持占位符原样输出
- 涉及人员资质或联系方式的条款，只需判断"是否提供了相关信息"
```
...
在脱敏之后、TOC 检测之前执行。提取的图片保存到 `/data/reviews/{id}/images/` 目录。

### 4b.2 docx 图片提取

1. 将 docx 作为 zip 打开，遍历 `word/media/` 目录
2. 通过 `word/_rels/document.xml.rels` 解析 rId → media 文件名映射
3. 通过 `word/document.xml` 解析 `<a:blip r:embed="rIdN">` 找到图片所在段落位置
4. 仅提取常见图片格式（png, jpg, jpeg, gif, bmp, tiff），跳过 emf/wmf 等矢量格式
5. 返回 `[{filename, path, near_para_index, content_type}]`

### 4b.3 PDF 图片提取

使用 pymupdf（fitz）的 `page.get_images()` + `doc.extract_image()` 逐页提取。如果 pymupdf 未安装，graceful degradation 返回空列表。

### 4b.4 图片标记注入

提取完成后，在含有图片的段落文本末尾注入标记 `[图片: filename]`，使 LLM 在审查时感知到图片的存在。例如：

- 原始段落："资质证书如下"
- 注入后："资质证书如下 [图片: image1.png]"

LLM prompt 中提示：存在 `[图片: filename]` 标记即视为已提供扫描件/证书。

### 4b.5 前端图片展示

- Preview API 在含图片的段落后注入 `<img>` 标签，图片 src 指向 `/api/reviews/{id}/images/{filename}`
- 新增 `GET /api/reviews/{id}/images/{filename}` 端点服务图片文件
- 安全措施：`os.path.realpath()` 验证路径不逃逸 images 目录
...
GET    /api/reviews/{id}/preview
  ...
  说明:
    - tender_html 由服务端用 _docx_to_html() 渲染，在高亮段落的 HTML 标签上
      注入 data-review-id="N" 属性和 CSS class（N 对应 review_items[].id）
    - 含图片的段落后注入 <img> 标签，src 指向 /api/reviews/{id}/images/{filename}
    - 未命中任何审查条目的段落正常渲染，无额外属性
    - summary.extracted_images 包含图片元数据列表
    - summary.pii_masked_count 记录脱敏的 PII 数量

GET    /api/reviews/{id}/images/{filename}
  Response: FileResponse (图片文件)
  安全: realpath 校验防止路径遍历
...
src/reviewer/                            — 审查核心模块（新目录）
  ├── __init__.py
  ├── desensitizer.py                    — PII 信息脱敏（姓名、电话、身份证、邮箱、银行账号）
  ├── image_extractor.py                 — 从 docx/PDF 提取嵌入图片
...
| 层级     | 范围                                                                            | 工具                       |
| ------ | ----------------------------------------------------------------------------- | ------------------------ |
| 单元测试   | desensitizer, image_extractor, toc_detector, clause_extractor, tender_indexer | pytest                   |
